...
public class CV extends Applet implements Runnable {
    ...
    while(true) {
      ...
      do {
        ...
        if (gameState == GAME_STATE_RISING) {
          ...
          if (draculaState == DRACULA_STATE_FADE_IN) {
            ...
            if (--draculaCounter == 0) {
              ...
              if (draculaPower >= 22
                  || (draculaPower > 10 && draculaPower < 16)
                  || (draculaPower < 6)) {
        ...
        if (playerJumping) {
          ...
          if (jumpReleased && a[VK_JUMP]
              && !a[VK_DOWN] && playerWhipping == 0 && playerDead == 0) {
            playerFy = playerY - 7;
            jumpReleased = false;
            playerJumping = true;
        ...
        if (playerDead > 0) {
          ...
          if (crosses.size() < 3 && subweaponReleased
              && a[VK_SUBWEAPON] && playerWhipping == 0) {
          ...
          if (playerWhipping > 0 && playerWhipping <= WHIP_EXTENDED
              && !playerThrowing) {
        ...
        if (playerStunned == 0 && draculaPower > 0
            && !playerHurt && draculaState != DRACULA_STATE_FADE_IN
            && draculaState != DRACULA_STATE_HIDDEN) {
          i = draculaX + (draculaReversed ? -8 : (draculaOpened ? -24 : -16));
          if (playerX2 >= i
              && playerX1 <= i + (draculaOpened ? 32 : 24)
              && playerY2 >= 128
              && playerY1 <= 176) {
            playerPower -= 2;
            playerHurt = true;
        ...
        o: for(i = queue.size() - 1; i >= 0; i--) {
          ...
          if (object[OBJ_TYPE] == TYPE_CROSS) {
            ...
            if (object[OBJ_STATE] == CROSS_STATE_FORWARD) {
              if ((object[OBJ_VX] < 0 && object[OBJ_X] <= 20)
                  || (object[OBJ_VX] > 0 && object[OBJ_X] >= 234)) {
            ...
            if (object[OBJ_Y] == 160) {
              if ((counter & 31) == 0) {
                object[OBJ_SPRITE] = object[OBJ_SPRITE] == SPRITE_IGOR_1
                    ? SPRITE_IGOR_2 : SPRITE_IGOR_1;
              }
              object[OBJ_REVERSED] = object[OBJ_X] > playerX ? 1 : 0;
              if (object[OBJ_COUNTER]-- == 0) {
                object[OBJ_COUNTER] = 32 + random.nextInt(64);
                object[OBJ_VY] = random.nextBoolean()
                    ? IGOR_JUMP_SPEED_1 : IGOR_JUMP_SPEED_2;
                object[OBJ_VX] = (object[OBJ_X] > playerX)
                    ? -IGOR_SPEED : IGOR_SPEED;
              }
            } else {
              object[OBJ_X] += object[OBJ_VX];
            }
          ...
          if (!playerHurt && draculaPower > 0) {
            if (object[OBJ_COLLIDES] == 1) {
              ...
              if (playerX2 >= object[OBJ_X] + object[OBJ_X1]
                  && playerX1 <= object[OBJ_X] + object[OBJ_X2]
                  && playerY2 >= object[OBJ_Y] + object[OBJ_Y1]
                ...
                } else if (playerStunned == 0
                    && (object[OBJ_TYPE] == TYPE_FIREBALL
                        || object[OBJ_TYPE] == TYPE_BRICK
                        || object[OBJ_TYPE] == TYPE_IGOR)) {
                  queue.remove(i);
                  float[] flame = new float[32];
                  queue.add(flame);
                  flame[OBJ_X] = object[OBJ_X] + object[OBJ_X1]
                        + ((object[OBJ_X2] - object[OBJ_X1] - 8) / 2);
                  flame[OBJ_Y] = object[OBJ_Y] + object[OBJ_Y1]
                      + ((object[OBJ_Y2] - object[OBJ_Y1] - 24) / 2);
                  flame[OBJ_SPRITE] = SPRITE_FLAME_1;
                  flame[OBJ_TYPE] = TYPE_FLAME;
              ...
              if (playerWhipping > 0 && playerWhipping <= WHIP_EXTENDED
                  && !playerThrowing
                  && whipX2 >= object[OBJ_X] + object[OBJ_X1]
                  && whipX1 <= object[OBJ_X] + object[OBJ_X2]
                  && whipY2 >= object[OBJ_Y] + object[OBJ_Y1]
                  && whipY1 <= object[OBJ_Y] + object[OBJ_Y2]) {
                if (object[OBJ_TYPE] == TYPE_FIREBALL
                    || object[OBJ_TYPE] == TYPE_DRACULA_HEAD
                    || object[OBJ_TYPE] == TYPE_IGOR) {
                  queue.remove(i);
                  float[] flame = new float[32];
                  queue.add(flame);
              ...
              if (object[OBJ_TYPE] == TYPE_FIREBALL
                ...
                for(j = 0; j < crosses.size(); j++) {
                  ...
                  if (cross[OBJ_X] + cross[OBJ_X2]
                          >= object[OBJ_X] + object[OBJ_X1]
                      && cross[OBJ_X] + cross[OBJ_X1]
                          <= object[OBJ_X] + object[OBJ_X2]
                      && cross[OBJ_Y] + cross[OBJ_Y2]
                          >= object[OBJ_Y] + object[OBJ_Y1]
                      && cross[OBJ_Y] + cross[OBJ_Y1]
      ...
      if (gameState != GAME_STATE_ENTRY
        ...
        if (draculaState == DRACULA_STATE_FADE_IN
          ...
          g.setColor(new Color(0xFFFFFF | (
              (draculaState == DRACULA_STATE_FADE_IN
                      ? draculaCounter : (draculaCounter >> 1))
                  << 24), true));
      ...
      for(i = 0; i < queue.size(); i++) {
        ...
        if (object[OBJ_SPRITE] >= 0) {
          ...
          g.drawImage(sprites[(int)object[OBJ_SPRITE]],
              (int)object[OBJ_DX], (int)object[OBJ_DY], null);
          g.setTransform(affineTransform);
        }
      }
      ...
      // draw fade
      if (playerDead > 64) {
        g.setColor(new Color(((playerDead >= 96) ? (127 - playerDead)
            : (playerDead - 64)) << 27, true));
  ...
  public void processKeyEvent(KeyEvent keyEvent) {
    ...
    if (k > 0) {
      a[(k == VK_LEFT || k == VK_RIGHT || k == VK_DOWN
          || k == VK_WHIP || k == VK_SUBWEAPON)
